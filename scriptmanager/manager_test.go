...
var _ = Describe("Manager", func() {
	...
	var (
		...
		url, user, repo, branch, scriptName, expected string
	)
...
echo "pandora opened"`
	...
	Context("When requesting a specific script", func() {
		var (
			scriptPath string
		)
			...
			BeforeEach(func() {
				fakeServer.AppendHandlers(
					ghttp.CombineHandlers(
						ghttp.VerifyRequest(
							"GET",
							"/zeus/pandora/master/some/specific/script.sh",
						),
						ghttp.RespondWith(
							http.StatusOK,
							expected,
				...
				scriptPath = "some/specific/script.sh"
				expected = `!/bin/bash
echo "pandora opened"`
			})

			It("should send the request", func() {
				_, err := scriptManager.GetScript(scriptPath)
				Expect(err).ToNot(HaveOccurred())
				Expect(fakeServer.ReceivedRequests()).Should(HaveLen(1))
			})
		...
		Context("When response status is other than 200 OK", func() {
			...
			It("should error", func() {
				_, err := scriptManager.Get(scriptPath)
				Expect(err).To(HaveOccurred())
			})

			It("should return an nil slice", func() {
				script, err := scriptManager.Get(scriptPath)
				Expect(err).To(HaveOccurred())
				Expect(script).To(BeNil())
			})
